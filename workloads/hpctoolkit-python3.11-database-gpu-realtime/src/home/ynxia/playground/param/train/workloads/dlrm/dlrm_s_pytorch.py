...
if __name__ == "__main__":
    ...
    ##################################################
    # import bgcp
    # from bgcp import storage
    # import bgcp_python
    # import bgcp_pytorch
    
    # bgcp.init()

    # print("BGCP initialized")
        
    # # 创建配置
    # profile_config = bgcp.config.ProfileConfig()
    
    # # 添加后端
    # python_config = bgcp_python.config.PythonConfig()
    # pytorch_config = bgcp_pytorch.config.PytorchConfig()
    # profile_config.add_backend(python_config)
    # profile_config.add_backend(pytorch_config)
    
    # import bgcp_cuda
    # cuda_config = bgcp_cuda.config.CudaConfig(activities=[
    #     "concurrent_kernel", "runtime", "memcpy", "memcpy2"
    # ])
    # profile_config.add_backend(cuda_config)
    
    # # Import user annotation library
    # import bgcp_tx as tx
    
    # # 创建输出
    # bgcp_writer = storage.FileWriter("./dlrm_bgcp_profile.pb")
    
    # # Add DCGM monitoring if needed (similar to your example)
    # try:
    #     import bgcp_dcgm
    #     field_id_list = [1001, 1002, 1003, 1005, 1006, 1007, 1008, 1009, 1010, 1011, 1012]
    #     gpu_id_list = [int(os.environ.get("RANK", "0"))]
    #     dcgm_config = bgcp_dcgm.config.DcgmConfig(gpu_id_list=gpu_id_list, field_id_list=field_id_list)
    #     profile_config.add_backend(dcgm_config)
    #     print("DCGM monitoring enabled")
    # except ImportError:
    #     print("DCGM not available, skipping...")
    
    # # Add writer and prepare for profiling
    # bgcp.add_writer(bgcp_writer)

    # print("BGCP config created and writer added")
    #########################################################
    ...
    if args.enable_profiling:
        print("Profiling enabled, will run for a few batches and exit...")
        # Use the modern torch.profiler
    ...
    # Start BGCP profiling
    # print("Starting BGCP profiling...")
    # bgcp.start_profile(profile_config)
    ...
    while k < args.nepochs:
        ...
        for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
            ...
            # Add iteration counter annotation
            # tx.counter(j + k * nbatches)
            '''
            # debug prints
            ...
            # with tx.annotate("forward_pass"):
            #     Z = dlrm_wrap(X, lS_o, lS_i, use_gpu, device)

            # loss
            E = loss_fn_wrap(Z, T, use_gpu, device)
            # with tx.annotate("loss_computation"):
            #     E = loss_fn_wrap(Z, T, use_gpu, device)
            '''
            # debug prints
            ...
            if not args.inference_only:
                ...
                # with tx.annotate("gradient_computation"):
                #     optimizer.zero_grad()
                #     # backward pass
                #     E.backward()
                ...
                # optimizer
                optimizer.step()
                # with tx.annotate("optimizer_step"):
                #     optimizer.step()
        ...
        k += 1  # nepochs
        # tx.mark("epoch_" + str(k) + "_completed")

    # Stop BGCP profiling
    # print("Stopping BGCP profiling...")
    # bgcp.stop_profile()
    # bgcp.remove_writer(bgcp_writer)
    # print("BGCP profiling complete. Data saved to ./dlrm_bgcp_profile.pb")

    #     # Analyze the collected data to generate Perfetto trace
    # print("\nAnalyzing collected data to generate Perfetto trace...")
    # try:
    #     import bgcp_bla
    #     # Create a reader for the raw data file we just created
    #     reader = storage.FileReader("./dlrm_bgcp_profile.pb")

    #     # Set analysis options
    #     analysis_config = {
    #         "output_dir": ".",  # Save the result in the current directory
    #         "concurrent_read": False
    #     }

    #     # Run the local analyzer
    #     bgcp_bla.local_analyze(analysis_config, [reader])
    #     print("✅ Analysis complete. Perfetto trace generated in the current directory.")
    #     print("You should now see a file named 'bgcp-....pb.gz' that can be opened in Perfetto UI.")

    # except ImportError:
    #     print("⚠️ bgcp_bla not found. Skipping analysis. You can analyze the raw file later using bgcp-parser.")
    # except Exception as e:
    #     print(f"❌ An error occurred during analysis: {e}")

    if profiler:
        profiler.stop()
        print("Profiling complete. Trace saved to ./profiler_output directory.")

    file_prefix = "%s/dlrm_s_pytorch_r%d" % (args.out_dir, ext_dist.my_rank)
    # profiling
    # The section for the old profiler is no longer needed.
    # if args.enable_profiling:
